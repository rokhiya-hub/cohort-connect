...
function Comment({ comment, videoId, currentUser, onDelete }) {
        ...
        {localReplies.length > 0 && (
          <div className="mt-3 space-y-3 pl-4 border-l border-gray-700">
            {localReplies.map((reply) => (
              <Comment key={reply._id} comment={reply} videoId={videoId} currentUser={currentUser} onDelete={onDelete} />
            ))}
          </div>
        )}
      </div>
...
export default function VideoCommentSection({ videoId, currentUser }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await api.post(`/videos/${videoId}/comments`, { content: text });
